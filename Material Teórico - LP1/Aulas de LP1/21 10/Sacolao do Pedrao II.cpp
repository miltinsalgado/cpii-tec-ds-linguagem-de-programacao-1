...
struct tVenda{
	...
	int contP=-1, contV = -1, contItem = -1;
	char nomePrV[20];
...
int buscaProduto (int produto)
{
	int achou = 0;
	
	for(int i = 0; i <= contP; i++)
	{
		if(prod[i].cod == produto)
		{
			...
			case '5':
				{
					for(int i=0; i < contV; i++)
					{
						...
						for(int j = 0; j < venda[i].qtdItens; j++)
							printf("\n %s - R$ %.2f - %.2f kg - R$ %.2f", PegaNome(prod[i].cod), venda[i].precoPrV[j], venda[i].qtdPrV[j], venda[i].precoPrV[j] * venda[i].qtdPrV[j]);
